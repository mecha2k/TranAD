...
class Attention(nn.Module):
    def __init__(self, feats):
        ...
        self.atts = [
            nn.Sequential(nn.Linear(self.n, feats * feats), nn.ReLU(True)) for i in range(1)
        ]
...
class OmniAnomaly(nn.Module):
    ...
    def forward(self, x, hidden=None):
        hidden = (
            torch.rand(2, 1, self.n_hidden, dtype=torch.float64) if hidden is not None else hidden
        )
...
class MTAD_GAT(nn.Module):
    ...
    def forward(self, data, hidden):
        hidden = (
            torch.rand(1, 1, self.n_hidden, dtype=torch.float64) if hidden is not None else hidden
        )
